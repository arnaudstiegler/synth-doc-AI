...
async def fetch_image(session, url, row_uid, detector, semaphore, writer, lock, csvfile):
...
async def process_images(file, semaphore, writer, lock, csvfile):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_image(session, row['url'], row['uid'], detector, semaphore, writer, lock, csvfile) for _, row in df.iterrows()]
