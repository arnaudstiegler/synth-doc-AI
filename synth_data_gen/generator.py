...
class GenericDocumentGenerator:
    ...
    def divide_rectangle(self, x, y, width, height, depth=0):
        ...
        if action == 'x':
            ...
            rectangles += self.divide_rectangle(x + width1, y, width2, height, depth+1)
            ...
            rectangles += self.divide_rectangle(x, y + height1, width, height2, depth+1)
            
        return rectangles

class UnstructuredDocumentGenerator(GenericDocumentGenerator):
    def generate(self):
        base_image = Image.new("RGBA", (int(self.BASE_WIDTH), int(self.BASE_WIDTH*self.A4_ASPECT_RATIO)), color="white")
        draw_base_image = ImageDraw.Draw(base_image)

        rectangles = self.divide_rectangle(0, 0, int(self.BASE_WIDTH), int(self.BASE_WIDTH*self.A4_ASPECT_RATIO))
        ...
        for rectangle in rectangles:
            ...
            if not component_generator.is_viable(rect_height, rect_width):
                draw_base_image.rectangle(((x_position, y_position), (x_position+rect_width, y_position+rect_height)), outline="black")
                continue
            new_component = component_generator.generate(int(rect_height), int(rect_width))
